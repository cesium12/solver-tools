...
def reverse_dict(d):
    '''
    Given a dictionary `d` that defines a one-to-one mapping,
    returns a dictionary `di` that defines the reverse mapping,
    i. e. ```di[d[k]]==k``` for each key `k` of `d`.
    '''
    return dict((v,k) for k, v in d.iteritems())

def _wsep_regex(wsep):
    if wsep is not None:
...
class Decoder(CoderCommon):
    ...
    def __init__(self,**kwargs):
        ...
        self.t_ANY_WORDSEP.__doc__ = _wsep_regex(self.wsep)
