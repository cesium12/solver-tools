...
def grid_layout(arr, width=75, height=10):
    ...
    padding = [cell_width] * layout.shape[1]
    for column in xrange(layout.shape[1],):
        for row in xrange(layout.shape[0],):
            entry = layout[row,column]
            trailing_spaces = len(entry) - len(entry.rstrip()) - 1
            if trailing_spaces < 0:
                trailing_spaces = 0
            if trailing_spaces < padding[column]:
                padding[column] = trailing_spaces

    def trim(row, padding):
        output = []
        for entry, p in zip(row, padding):
            if p == 0:
                output.append(entry)
            else:
                output.append(entry[:-p])
        return output

    ungrid = [trim(row, padding) for row in layout]
...
def array_to_string(arr, width=75, height=10):
    ...
    while arr.ndim < 2:
        arr = arr[np.newaxis, ...]
    return layout_to_string(grid_layout(arr, width, height)[0])
